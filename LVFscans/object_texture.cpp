...
void init_texture(const int width, const int height, GLuint &tt, unsigned char * TEXTURE_DATA, GLUquadric *tmpquad)
{
  glGenTextures(1, &tt);
  ...
  tmp_quad = gluNewQuadric();

  glEnable(GL_TEXTURE_2D);
  glBindTexture( GL_TEXTURE_2D, tt);

  gluQuadricTexture(tmp_quad,1);
