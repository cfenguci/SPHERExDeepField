...
{
char filein[96],fileout[]="Mytest.gits";
...
*/
    fits_open_file(&afptr, filein, READONLY, &status); /* open input images */
    if (status) {
...
check=1;
    ...
    if (check && !fits_create_file(&outfptr, fileout, &status) )
    {
...
for (ii = 0; ii < npixels; ii++) image[ii][firstpix[1]-1][firstpix[2]-1]=apix[ii];
